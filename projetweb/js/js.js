...
function fetchData() {
  ...
  articlesContainer.className = "articles__items";
  ...
  for (i = startIndex; i < endIndex; i++) {
    (function (index) { // Utilisez une IIFE pour capturer la valeur actuelle de i
      ...
      let backgroundImage = x[index].getElementsByTagName("image")[0].childNodes[0].nodeValue;
      article.style.background = "url('" + backgroundImage + "')";
      article.style.backgroundSize = "cover";

      let filterDiv = document.createElement("div");
      filterDiv.className = "article__filter";

      let checkBox = document.createElement("input");
      checkBox.type = "checkbox";
      checkBox.value = x[i].getElementsByTagName("ID")[0].textContent; // Assurez-vous de définir une valeur pour chaque case à cocher
      checkBox.addEventListener('change', function (event) {
        setFavorite(event);
      });

      let iconDiv = document.createElement("div");
      iconDiv.className = "article__icon";

      let button = document.createElement("button");
      button.className = "btn btn-danger";
      button.innerHTML = "En savoir +";
      button.onclick = function () {
        // Utilisez la variable index au lieu de i
        window.location.href = 'Biens/bien' + x[index].getElementsByTagName("ID")[0].childNodes[0].nodeValue + '.html';
      };

      iconDiv.appendChild(button);
      article.appendChild(filterDiv);
      article.appendChild(iconDiv);

      iconDiv.appendChild(checkBox)
      articlesContainer.appendChild(article);
    })(i); // Passez la valeur actuelle de i à la fonction immédiate
  }
